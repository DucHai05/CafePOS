...
const OrderPage = () => {
    ...
    const [itemsToPrint, setItemsToPrint] = useState([]); // Lưu những gì thực sự cần in (Thêm/Hủy)
        ...
        // --- 4. HELPER FUNCTIONS ---
    const calculateDiscountValue = (promo, amount) => {
        if (!promo) return 0;
        return promo.loaiKhuyenMai === 'PERCENT' ? (amount * promo.giaTri) / 100 : promo.giaTri;
    };
    // --- 5. TÍNH TOÁN TIỀN ---
   ...
   // --- 7. HANDLERS (HÀM CHỐT THANH TOÁN) ---
    const handlePaymentConfirm = async (method) => {
            console.log(">>> Bắt đầu luồng thanh toán nhanh...");
            
            try {
                // 1. ĐỊNH NGHĨA orderData (Cái này Hải thiếu nè)
                const orderData = { 
                    maHoaDon: maHoaDon, 
                    maBan: maBan, 
                    items: cart.map(item => ({
                ...
                // 2. CẬP NHẬT ĐƠN HÀNG LÊN SERVER TRƯỚC (Để đảm bảo giá tiền và món là mới nhất)
                console.log(">>> Đang đồng bộ đơn hàng trước khi thanh toán...");
                const createRes = await orderApi.staffCreate(orderData);
                
                // Lấy mã hóa đơn từ kết quả trả về (Lưu ý: Hải check xem Backend trả về .hoaDon hay .maHoaDon trực tiếp nhé)
                // Nếu Backend trả về Map.of("hoaDon", savedHD) thì dùng dòng dưới:
                ...
                const paymentPayload = {
                    ...
                    thoiGianThanhToan: new Date().toISOString(), // Dùng ISO cho chuẩn DB
                ...
                if (payRes.status === 200 || payRes.status === 201) {
                    // Đổi màu bàn sang XANH (Service 8083)
                    await tableApi.updateTrangThai(maBan, 'PAID');
                    
                    alert(`Thanh toán thành công! Mã đơn: ${currentMaHD}`);
